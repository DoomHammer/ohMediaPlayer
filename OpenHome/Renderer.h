...
public:
    ...
    virtual void Next(uint32_t aHandle, uint32_t aAfterId, uint32_t& aId, std::string& aUri) = 0;
    ...
    virtual void Metatext(uint32_t aHandle, uint32_t aId, const std::string& aDidlLite) = 0;
...
{
public:
    virtual void Play(uint32_t aHandle, uint32_t aId, const std::string& aUri, uint32_t aSecond) = 0;
